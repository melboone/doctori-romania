 ...
 * @ingroup themeable
 */
function SimpleBlue_menu_local_task($mid, $active, $primary) {
  $active_class = "";
  if ($active) {
    $active_class .= "active ";
  }
 ...
 *   The url of the feed.
 */
function SimpleBlue_feed_icon($url) {
 ...
 */
function SimpleBlue_comment_wrapper($content, $type = null) {
  static $node_type;
 ...
 */
function SimpleBlue_breadcrumb($breadcrumb) {
  if (!empty($breadcrumb)) {
    return '<div class="breadcrumb art-postcontent">'. implode(' | ', $breadcrumb) .'</div>';
  }
}

function SimpleBlue_service_links_node_format($links) {
 ...
 * @ingroup themeable
 */
function SimpleBlue_button($element) {
...
*/
function SimpleBlue_img_assist_page($content, $attributes = NULL) {
